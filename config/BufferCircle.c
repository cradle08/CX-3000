...
#ifndef  __BUFFER_CIRCLE_C__
#define  __BUFFER_CIRCLE_C__
...
_STA_ IO_ UINT8  XRAM_ m_send_achBufferCircle[BUFFER_CIRCLE_LENGTH_SEND];

// for receiving chars
_STA_ IO_ struct tBufferCircle XRAM_ m_recv_tBufStatus = {0, 1}; // , e_False}; // attention: initialized value
_STA_ IO_ UINT8  XRAM_ m_recv_achBufferCircle[BUFFER_CIRCLE_LENGTH_RECV];
...
//
UINT8  BUF_Send_Init(void)
{
    // attention: 1. nGet < BUFFER_CIRCLE_LENGTH_SEND
    //            2. nPut < BUFFER_CIRCLE_LENGTH_SEND
    //            3. empty: nPut = nGet + 1
...
//
UINT8  BUF_Send_PutChar(UINT8 ch)
{
#if 0
    // busy
	...
	if(m_send_tBufStatus.nPut >= BUFFER_CIRCLE_LENGTH_SEND)
	{
...
//
UINT8  BUF_Send_GetChar(UINT8* pch)
{
#if 0
    // busy
    if(e_True == m_send_tBufStatus.eBusy)
	...
	if( ( m_send_tBufStatus.nPut == (m_send_tBufStatus.nGet+1) ) ||  
        ( (0==m_send_tBufStatus.nPut) && ((BUFFER_CIRCLE_LENGTH_SEND-1)==m_send_tBufStatus.nGet) ) )
	{
	    // m_send_tBufStatus.eBusy = e_False;
        return e_Feedback_Fail;
	}
	...
	if(m_send_tBufStatus.nGet >= BUFFER_CIRCLE_LENGTH_SEND)
	{
...
//
UINT8  BUF_Send_PutNChar(UINT8* pchBuf, UINT16 nLen)
{
...
//
UINT8  BUF_Send_GetNChar(UINT8* pchBuf, UINT16 nLen)
{
    IO_ UINT16 XRAM_ nCount = 0;
#if 0	
    // busy
...
//
UINT16 BUF_Send_GetLenRemain(void)
{
	...
	// 2. the buffer is empty
             ...
             ( (0==m_send_tBufStatus.nPut) && ((BUFFER_CIRCLE_LENGTH_SEND-1)==m_send_tBufStatus.nGet) ) )
	{
        return (BUFFER_CIRCLE_LENGTH_SEND-1);
	}
	...
	else 
	{
        ...
        if(m_send_tBufStatus.nPut > (m_send_tBufStatus.nGet+1) )
        {
            nLen = (BUFFER_CIRCLE_LENGTH_SEND + m_send_tBufStatus.nGet) - m_send_tBufStatus.nPut;
		}
		else  // .nPut < .nGet
		{
...
#else
    nLen = BUF_Send_GetLenConsume();
    return ((BUFFER_CIRCLE_LENGTH_SEND-1) - nLen);
#endif	
}

//
UINT16 BUF_Send_GetLenConsume(void)
{
    IO_ UINT16 XRAM_ nLen = 0;
    // attention: the flag ".eBusy" is not checked here !
	...
	else 
	{
		...
		{
			nLen = (BUFFER_CIRCLE_LENGTH_SEND+m_send_tBufStatus.nPut) - (m_send_tBufStatus.nGet+1);
		}
		//
		return nLen;
	}
...
#if 1
//
UINT8  BUF_Recv_Init(void)
{
    // attention: 1. nGet < BUFFER_CIRCLE_LENGTH_RECV
    //            2. nPut < BUFFER_CIRCLE_LENGTH_RECV
    //            3. empty: nPut = nGet + 1
...
//
UINT8  BUF_Recv_PutChar(UINT8 ch)
{
#if 0
    // busy
	...
	if(m_recv_tBufStatus.nPut >= BUFFER_CIRCLE_LENGTH_RECV)
	{
...
//
UINT8  BUF_Recv_GetChar(UINT8* pch)
{
#if 0
    // busy
    if(e_True == m_recv_tBufStatus.eBusy)
	...
	if( ( m_recv_tBufStatus.nPut == (m_recv_tBufStatus.nGet+1) ) ||  
        ( (0==m_recv_tBufStatus.nPut) && ((BUFFER_CIRCLE_LENGTH_RECV-1)==m_recv_tBufStatus.nGet) ) )
	{
	    // m_recv_tBufStatus.eBusy = e_False;
        return e_Feedback_Fail;
	}
	...
	if(m_recv_tBufStatus.nGet >= BUFFER_CIRCLE_LENGTH_RECV)
	{
...
//
UINT8  BUF_Recv_PutNChar(UINT8* pchBuf, UINT16 nLen)
{
    IO_ UINT16 XRAM_ nCount = 0;
#if 0	
    // busy
...
//
UINT8  BUF_Recv_GetNChar(UINT8* pchBuf, UINT16 nLen)
{
    IO_ UINT16 XRAM_ nCount = 0;
#if 0	
    // busy
...
//
UINT16 BUF_Recv_GetLenRemain(void)
{
    IO_ UINT16 XRAM_ nLen = 0;
    // attention: the flag ".eBusy" is not checked here !
	...
	// 2. the buffer is empty
             ...
             ( (0==m_recv_tBufStatus.nPut) && ((BUFFER_CIRCLE_LENGTH_RECV-1)==m_recv_tBufStatus.nGet) ) )
	{
        return (BUFFER_CIRCLE_LENGTH_RECV-1);
	}
	// 3. the buffer is part used
	else 
	{
        ...
        if(m_recv_tBufStatus.nPut > (m_recv_tBufStatus.nGet+1) )
        {
            nLen = (BUFFER_CIRCLE_LENGTH_RECV + m_recv_tBufStatus.nGet) - m_recv_tBufStatus.nPut;
		}
		else  // .nPut < .nGet
		{
...
#else
    nLen = BUF_Recv_GetLenConsume();
    return ((BUFFER_CIRCLE_LENGTH_RECV-1) - nLen);
#endif	
}

//
UINT16 BUF_Recv_GetLenConsume(void)
{
    IO_ UINT16 XRAM_ nLen = 0;
    // attention: the flag ".eBusy" is not checked here !
	...
	else 
	{
		...
		{
			nLen = (BUFFER_CIRCLE_LENGTH_RECV+m_recv_tBufStatus.nPut) - (m_recv_tBufStatus.nGet+1);
		}
		//
		return nLen;
	}
