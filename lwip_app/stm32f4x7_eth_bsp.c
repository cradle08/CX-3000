...
void ETH_BSP_Config(void)
{
  ...
  *****************************************************************************/
  ...
  if( !(ETH_ReadPHYRegister(LAN8720_PHY_ADDRESS, 29) & 0x10) )   // get the link status, 1 meaning link down --- index: 29, mark: 0x10
  {
...
void ETH_GPIO_Config(void)
{
    ...
    EVAL_OutputClr(O_LAN8720_RST);  //硬件复位LAN8720
	IT_SYS_DlyMs(100);
	EVAL_OutputSet(O_LAN8720_RST);  //复位结束 
}
...
void ETH_link_callback(struct netif *netif)
{
 ...
 uint32_t tmpreg,RegValue;
  ...
  if(netif_is_link_up(netif))
  {
    ...
    //
    printf("\r\n Link callback: Network Cable is connected now \r\n");
    ...
    iptab[2] = IP_ADDR1;
    iptab[3] = IP_ADDR0;
    printf("\r\n Link callback: Static IP address: %d.%d.%d.%d \r\n", iptab[3], iptab[2], iptab[1], iptab[0]); 
  #endif /* USE_DHCP */
  ...
  else
  {
    ...
    printf("\r\n Link callback: Network Cable is unplugged \r\n");
